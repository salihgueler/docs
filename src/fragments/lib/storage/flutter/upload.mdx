...
To upload to S3 from a file, specify the `path` and the `localFile` to be uploaded, where the `localFile` can be an instance of `AWSFile` created from either an OS platform `File` instance or the result of Flutter file picker plugins such as [file_picker](https://pub.dev/packages/file_picker).
...
Future<void> uploadIOFile(io.File file) async {
  ...
  try {
    final uploadResult = await Amplify.Storage.uploadFile(
      localFile: awsFile,
      path: StoragePath.fromString('upload/file.png'),
    ).result;
...
Future<void> uploadFileWithPath(localFile file, String path) async {
  final result = await Amplify.Storage.uploadFile(
    localFile: file,
    path: (id) => 'protected/$id/picture.png',
  ).result;
}
```
...
Future<void> uploadImage() async {
  ...
  try {
    final result = await Amplify.Storage.uploadFile(
      ...
      path: (id) => 'protected/$id/${platformFile.name}',
      onProgress: (progress) {
...
To upload to S3 from a data object, specify the `path` and `data`, where `data` is an instance of `S3DataPayload` created from various data formats.
...
Future<void> uploadWithOptions() async {
  ...
  final uploadDataOperation = Amplify.Storage.uploadData(
    ...
    path: StoragePath.fromString('public/example.txt'),
...
Future<void> uploadFile(String path) async {
  try {
    ...
    safePrint('Uploaded ${result.uploadedItem.path}');
  } on StorageException catch (e) {
    safePrint('Error uploading file: ${e.message}');
  }
}
