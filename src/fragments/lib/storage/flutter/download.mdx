...
Future<void> downloadToLocalFile(StoragePath path) async {
...
```dart
Future<void> downloadToLocalFileOnWeb(StoragePath path) async {
  try {
    final result = await Amplify.Storage.downloadFile(
      path: path
...
```dart
Future<void> downloadToMemory(StoragePath path) async {
  try {
